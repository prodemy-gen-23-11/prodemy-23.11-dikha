...
function AddProducts() {
    ...
    const { data: database, isLoading } = useSWR("http://localhost:3000/products", getAllProducts);

    const schema = yup.object().shape({
        nama: yup.string().required("Nama Masih Kosong"),
        harga: yup.number().positive().integer().required("Masukan Harga Keliru"),
        stok: yup.number().positive().integer().required("Masukan Stok Keliru"),
        deskripsi: yup
            .string()
    ...
    const onSubmitForm = (data) => {
        ...
        const { nama, harga, deskripsi, stok, ...objGambar } =
            data;
